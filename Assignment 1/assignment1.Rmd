...
(2)
For (i) we should add the number of mutual edges, for (ii) the number of transitive two-path, considering decreasing weight of triangles to avoid near-degeneracy, for (iii) 
...
```{r}
set.seed(1234)
fit.ergm3 <- ergm(friend_net ~ nodematch('sex') + 
                    nodeofactor('sex') + 
                    nodeifactor('smoke') +
                    nodematch('activity'))
summary(fit.ergm3)
...
```{r}
summary(ergm(friend_net ~ edges + nodematch('sex')))
```
