...
Without the structural terms, the \texttt{ergm} function uses MPLE, so the estimates are the same as in Task 1 (if we also include the edge parameter), but the p-values are different. In particular, now we only reject the hypothesis that smokers are more likely to receive friendship nominations than non-smokers.
